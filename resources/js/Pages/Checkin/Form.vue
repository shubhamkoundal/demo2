  ...
  <div class="flex flex-col gap-6">
    <div class="flex flex-col lg:flex-row gap-6">
      <div class="flex flex-col gap-6 w-full lg:w-1/2">
        ...
        <!-- Dropdown to choose between customer and supplier -->
        <div class="col-span-6 sm:col-span-4 relative mb-2">
          <label class="block text-sm font-medium text-gray-700">
            {{ $t('Select Type') }}
          </label>
          <select v-model="selectedType" @change="handleChange" class="pr-8 mt-2 w-full block text-base py-2 pl-4 placeholder-gray-400 border border-gray-300 focus:outline-none focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm">
        ...
        <!-- Conditional rendering based on selected type -->
          ...
          <!-- Autocomplete dropdown for customers -->
          <auto-complete
               id="customer"
                :label="$t('Customer')"
        ...
        <div v-else-if="selectedType === 'supplier'">
          <!-- Autocomplete dropdown for suppliers -->
          <auto-complete id="supplier" :label="$t('Supplier')" :suggestions="suppliers" v-model="form.supplier_id" />
        </div>
            </div>
          </div>
    ...
    When a <span style="font-weight: bold;">MAC address</span> is provided, the <span style="font-weight: bold;">Quantity</span> should always be <span style="font-weight: bold; color: red;">1</span>. <br>
    <span style="color: #999; font-style: italic;">Note: Ensure this rule is followed consistently.</span>
</p>
              <div
              ...
              <div v-else>
                <div
                  ...
                  :class="{
                    ...
                    error:
                      ...
                      $page.props.errors['items.' + ii + '.mac_address'] ||
                      $page.props.errors['items.' + ii + '.serial_number'] ||
                      $page.props.errors['items.' + ii + '.warranty_end_date'] ||
                ...
                >
                    ...
                    <div v-for="v in item.selected.variations" :key="v.sku">
                      ...
                      <div class="w-full block sm:flex items-center justify-stretch gap-4">
                        ...
                        <text-input 
                          ...
                          :label="$t('Mac Address')"
                          class="mt-4 w-full sm:w-1/3"
                          v-if="$settings.mac_address == 1 && item.mac_address !== false && item.mac_address !== null"
                        />
                         <text-input 
                          type="text" 
                          ...
                          class="mt-4 w-full sm:w-1/3"
                          v-if="$settings.serial_number == 1 && item.serial_number !== false && item.serial_number !== null"
                        />
                         <text-input 
                          type="date" 
                          ...
                          :label="$t('Weight')"
                          class="mt-4 w-full sm:w-1/3"
                          v-if="$settings.warranty_end_date == 1 && item.warranty_end_date !== false && item.warranty_end_date !== null"
                        />
                      </div>
                    </div>
                  </template>
                  ...
                  <div class="mt-4">
                    ...
                    <div v-if="$page.props.errors['items.' + ii + '.mac_address']" class="text-red-600 pt-1 rounded-md">
                      {{ $page.props.errors['items.' + ii + '.mac_address'].split('when').shift() }}.
                    </div>
                    <div v-if="$page.props.errors['items.' + ii + '.serial_number']" class="text-red-600 pt-1 rounded-md">
                      {{ $page.props.errors['items.' + ii + '.serial_number'].split('when').shift() }}.
                    </div>
                    <div v-if="$page.props.errors['items.' + ii + '.warranty_end_date']" class="text-red-600 pt-1 rounded-md">
                      {{ $page.props.errors['items.' + ii + '.warranty_end_date'].split('when').shift() }}.
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div v-else class="bg-white mt-4 rounded-md shadow overflow-x-auto">
              <table class="w-full">
                <tr class="text-left font-bold">
                  ...
                  <!--<th class="px-2 lg:px-6 py-4">{{ $t('Mac Address') }}</th>-->
                  <th class="px-2 lg:px-6 py-4 text-center" :class="$settings.mac_address ? 'w-32 xl:w-56' : 'w-px'">
                    <span v-if="$settings.mac_address">{{ $t('Mac Address') }}</span>
                  </th>
                  <th class="px-2 lg:px-6 py-4 text-center" :class="$settings.serial_number ? 'w-32 xl:w-56' : 'w-px'">
                    <span v-if="$settings.serial_number">{{ $t('Serial Number') }}</span>
                  </th>
                  <th class="px-2 lg:px-6 py-4 text-center" :class="$settings.warranty_end_date ? 'w-32 xl:w-56' : 'w-px'">
                    <span v-if="$settings.warranty_end_date">{{ $t('Warranty End Date') }}</span>
                  </th>
                  <!--<th class="px-2 lg:px-6 py-4">{{ $t('Serial Number') }}</th>-->
                  <!--<th class="px-2 lg:px-6 py-4">{{ $t('Warranty End Date') }}</th>-->
                ...
                <template v-if="form.items.length">
                  <template v-for="(item, ii) in form.items" :key="item.id">
                    <template v-if="item.selected && item.selected.variations && item.selected.variations.length">
                      <tbody class="group">
                        ...
                        >
                          ...
                          <td>
                            <div class="px-2 xl:px-6 pb-2 text-right" v-if="$settings.mac_address == 1 && item.mac_address == 1">
                              <text-input type="text" v-model="variation.mac_address" size="small" class="w-full block" />
                            </div>
                          </td>
                          <td>
                            <div class="px-2 xl:px-6 pb-2 text-right" v-if="$settings.serial_number == 1 && item.serial_number == 1">
                              <text-input type="text" v-model="variation.serial_number" size="small" class="w-full block" />
                            </div>
                          </td>
                          <td>
                            <div class="px-2 xl:px-6 pb-2 text-right" v-if="$settings.warranty_end_date == 1 && item.warranty_end_date == 1">
                              <text-input type="date" v-model="variation.warranty_end_date" size="small" class="w-full block" />
                            </div>
                          </td>
                          <td>
                    ...
                    >
                     ...
                     <td class="border-t">
                      <div class="px-2 xl:px-6 py-2 text-right" v-if="$settings.mac_address == 1 && item.mac_address !== false && item.mac_address !== null">
                         <text-input type="text" v-model="item.mac_address" class="w-full block" />
                        </div>
                      </td>
                      <td class="border-t">
                      <div class="px-2 xl:px-6 py-2 text-right" v-if="$settings.serial_number == 1 && item.serial_number !== false && item.serial_number !== null">
                         <text-input type="text" v-model="item.serial_number" class="w-full block" />
                        </div>
                      </td>
                    <td class="border-t">
                      <div class="px-2 xl:px-6 py-2 text-right" v-if="$settings.warranty_end_date == 1 && item.warranty_end_date !== false && item.warranty_end_date !== null">
                         <text-input type="date" v-model="item.warranty_end_date" :error="$page.props.errors.warranty_end_date" class="w-full block" />
                        </div>
                      </td>
                      <td class="border-t">
...
export default {
  props: ['edit', 'contacts', 'warehouses', 'suppliers', 'customers'],
  components: {
    Dialog,
    CheckBox,
    TextInput,
  ...
  data() {
    return {
      selectedType: this.edit ? (this.edit.customer_id ? 'customer' : 'supplier') : 'supplier', // Initialize based on existing data
      files: [],
      item: {
        mac_address: '', // Initialize mac_address
        serial_number: '',
        warranty_end_date: '',
      },
      confirm: false,
 ...
 created() {
  if (this.edit) {
    this.form.items = this.edit.items.map(i => {
      let macAddress = i.mac_address !== null ? i.mac_address : false;
      let serialNumber = i.serial_number !== null ? i.serial_number : false;
      let warrantyEndDate = (i.warranty_end_date !== null && i.warranty_end_date !== false && i.warranty_end_date !== '') ? i.warranty_end_date : false; // Set to true if originally null or empty string

      // Convert 0 to false for mac_address
      if (macAddress == 0) {
        macAddress = false;
      }

      // Convert 0 to false for serial_number
      if (serialNumber == 0) {
        serialNumber = false;
      }
  ...
  methods: {
    ...
    itemSelected(v) {
      ...
      let existingItem = this.form.items.find(i => i.id === v.id);

      if (existingItem) {
        macAddress = existingItem.mac_address;
        serialNumber = existingItem.serial_number;
        warrantyEndDate = existingItem.warranty_end_date;
      } else {
        macAddress = (this.$settings.mac_address == 1 && v.mac_address !== false && v.mac_address !== null) ? '' : v.mac_address;
        serialNumber = (this.$settings.serial_number == 1 && v.serial_number !== false && v.serial_number !== null) ? '' : v.serial_number;
        warrantyEndDate = (this.$settings.warranty_end_date == 1 && v.warranty_end_date !== false && v.warranty_end_date !== null) ? '' : v.warranty_end_date;
      }
      ...
      if (v.has_variants && v.variants.length > 0) {
        ...
        if (existingItem) {
          existingItem.mac_address = macAddress;
          existingItem.serial_number = serialNumber;
          existingItem.warranty_end_date = warrantyEndDate;
          existingItem.quantity += 1;
          existingItem.weight += (this.$settings.track_weight == 1 && existingItem.track_weight == 1) ? 1 : 0;
        } else {
          this.form.items.push({
            ...v,
            quantity: 1,
            unit_id: v.unit_id,
            weight: (this.$settings.track_weight == 1 && v.track_weight == 1) ? 1 : 0,
            mac_address: macAddress,
            serial_number: serialNumber,
            warranty_end_date: warrantyEndDate,
          });
    ...
    submit() {
    if (this.item.warranty_end_date) {
      this.item.warranty_end_date = this.formatDate(this.item.warranty_end_date);
    } else {
      this.item.warranty_end_date = null;
    }
    if (this.$refs.file) {
    ...
    this.form
      .transform(data => ({
        ...
        items: data.items.map(i => ({
          ...
          warranty_end_date: i.warranty_end_date || null,
        })),
      }))
