<template>
  <admin-layout :title="$t('import_x', { x: $t('Customers') })">
    <tec-form-section @submitted="submit">
      <template #title>{{ $t('import_x', { x: $t('Customers') }) }}</template>
          ...
          >
            <div class="space-y-1 text-center">
              ...
              <div class="text-xs text-gray-700">
                <div>{{ $t('Excel file should have name, phone, email,customerId,isp & details columns.') }}</div>
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      this.form.post(route('customers.import.save'), { preserveScroll: true });
    },
  },
};
</script>
