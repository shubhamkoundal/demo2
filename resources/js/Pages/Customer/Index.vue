<template>
  <admin-layout :title="$t('Customers')">
    <div class="px-4 md:px-0">
      <tec-section-title class="-mx-4 md:mx-0 mb-6">
        <template #title>{{ $t('Customers') }}</template>
      ...
      <div class="mb-6 flex justify-between items-center print:hidden">
        ...
        <tec-dropdown align="right" width="48" v-if="$can(['create-customers', 'import-customers', 'export-customers'])">
          <template #trigger>
            <button
          ...
          <template #content>
            <tec-dropdown-link v-if="$can('create-customers')" :href="route('customers.create')">
            ...
            >
              {{ $t('export_x', { x: $t('Customers') }) }}
            </a>
            <tec-dropdown-link v-if="$can('import-customers')" :href="route('customers.import')">
              {{ $t('import_x', { x: $t('Customers') }) }}
            </tec-dropdown-link>
          </template>
        </tec-dropdown>
      </div>
      <div class="bg-white -mx-4 md:mx-0 md:rounded-md shadow overflow-x-auto">
        <table class="w-full whitespace-nowrap">
          <tr class="text-left font-bold">
            <th class="px-6 pt-6 pb-4">{{ $t('CustomerId') }}</th>
            <th class="px-6 pt-6 pb-4">{{ $t('Name') }}</th>
            <th class="px-6 pt-6 pb-4">{{ $t('Email') }}</th>
            <th class="px-6 pt-6 pb-4">{{ $t('Phone') }}</th>
            <th class="px-6 pt-6 pb-4">{{ $t('ISP') }}</th>
            <th class="px-6 pt-6 pb-4" colspan="2">{{ $t('Details') }}</th>
          </tr>
          <tr
            :key="customer.id"
            v-for="customer in customers.data"
            @click="goto(route('customers.edit', customer.id))"
            class="hover:bg-gray-100 focus-within:bg-gray-100"
            :class="{ 'cursor-pointer': $can('update-customers') }"
          >
               <td class="border-t">
              <div class="px-6 py-4 flex items-center focus:text-indigo-500">
                {{ customer.customerId }}
                <icons v-if="customer.deleted_at" name="trash" class="shrink-0 w-4 h-4 text-red-500 ml-2" />
              </div>
            </td>
            <td class="border-t">
              <div class="px-6 py-4 flex items-center focus:text-indigo-500">
...
<!--</td>-->
            ...
            <td class="border-t w-px">
              <div v-if="$can('update-customers')" class="px-4 flex items-center">
      ...
      <pagination class="mt-6" :meta="customers.meta" :links="customers.links" />
    </div>
  </admin-layout>
</template>
  ...
  watch: {
    form: {
      handler: throttle(function () {
        ...
        this.$inertia.visit(this.route('customers.index', Object.keys(query).length ? query : { remember: 'forget' }), {
          onFinish: () => {
