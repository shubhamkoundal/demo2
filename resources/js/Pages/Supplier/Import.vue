<template>
  <admin-layout :title="$t('import_x', { x: $t('Suppliers') })">
    <tec-form-section @submitted="submit">
      <template #title>{{ $t('import_x', { x: $t('Suppliers') }) }}</template>
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      this.form.post(route('suppliers.import.save'), { preserveScroll: true });
    },
  },
};
</script>
