<template>
  <admin-layout :title="$t(edit ? 'edit_x' : 'create_x', { x: $t('Supplier') })">
    <tec-form-section @submitted="update">
      <template #title>
        <div class="flex items-center">
          <template v-if="edit">
            <inertia-link class="text-blue-600 hover:text-blue-700" :href="route('suppliers.index')">{{ $t('Suppliers') }}</inertia-link>
...
export default {
  ...
  methods: {
    update() {
      this.form.post(this.edit ? this.route('suppliers.update', this.edit.id) : this.route('suppliers.store'), {
        preserveScroll: true,
    ...
    deleteSupplier() {
      this.form.delete(route('suppliers.destroy', this.edit.id), {
    ...
    restoreSupplier() {
      this.$inertia.put(this.route('suppliers.restore', this.edit.id), {
    ...
    deleteSupplierPermanently() {
      this.form.delete(route('suppliers.destroy.permanently', this.edit.id), {
        onSuccess: () => this.closeModal(),
      });
    },
    closePermanentModal() {
